...
This project require the dateset from https://manning.app.box.com/s/rm9b4ksyiqlw6wu66vahmonubd8ixgq4.
It should be located in a folder realtive to this file ./02_data/
"""
...
IMAGE_PATH = "ObjectDetection/01_data/self_driving_datasets/"


def filter_missing_frames(e):
    image_name = e["frame"]
    return os.path.exists(IMAGE_PATH + image_name)


def preprocess_dataset(e):
    image_name = e["frame"]

    e["image"] = Image.open(IMAGE_PATH + image_name)
...
if __name__ == "__main__":
    ...
    # Prepare data
    dataset_raw = datasets.load_dataset("ObjectDetection/01_data")

    dataset_raw["train"]

    print(dataset_raw)
    dataset_raw = dataset_raw.filter(filter_missing_frames)
    print(dataset_raw)
    dataset_raw = dataset_raw.map(preprocess_dataset)
    ...
    unique_labels = len(np.unique(dataset_raw["train"][label_column]))
    dataset_onehot = dataset_raw.map(
        lambda x: {
            "target": [0 if v != x[label_column] else 1 for v in range(unique_labels)]
        }
    )

    dataset = dataset_onehot.remove_columns(label_column)
    dataset = dataset.remove_columns("frame")
